...
def _flat(value: float, at: tuple[float, float] = (40.7, -105.5)) -> dict:
    return {at: value}
...
def test_temperatures_arrive_in_fahrenheit_and_cloud_becomes_clear_sky(cache):
    ...
    values = NasaPowerClimateSource(cache).fetch((FORT_COLLINS,))
    assert values["hottest_day"]["08069"] == pytest.approx(104.0)
    assert values["coldest_day"]["08069"] == pytest.approx(14.0)
    # Cloud amount is what the service reports; "clear sky" is its complement.
    assert values["sunshine"]["08069"] == pytest.approx(70.0)
    assert values["humidity"]["08069"] == pytest.approx(55.0)
...
def test_the_nearest_search_widens_past_an_empty_cell(cache):
    ...
    coastal = Place(
        fips="08059", name="Jefferson", state="Colorado", lat=41.6, lon=-105.6, population=1
    )
    assert "08059" in NasaPowerClimateSource(cache).fetch((coastal,))["humidity"]
...
def test_the_closest_point_wins_when_several_are_in_range(cache):
    ...
    _seed(
        cache,
        (40.0, -110.0),
        {p: {(40.7, -105.5): 10.0, (41.4, -105.9): 90.0} for p in PARAMETERS},
    )
    assert NasaPowerClimateSource(cache).fetch((FORT_COLLINS,))["humidity"][
        "08069"
    ] == pytest.approx(10.0)
...
def test_places_are_covered_by_tiles_on_a_fixed_lattice():
    ...
    tiles = NasaPowerClimateSource._tiles((FORT_COLLINS, MIAMI))
    assert tiles == sorted(tiles)
    for place in (FORT_COLLINS, MIAMI):
        assert any(
...
def test_two_places_in_one_tile_are_downloaded_once():
    ...
    neighbour = Place(
        fips="08013", name="Boulder", state="Colorado", lat=40.09, lon=-105.36, population=1
    )
    assert len(NasaPowerClimateSource._tiles((FORT_COLLINS, neighbour))) == 1
...
def test_a_cached_tile_is_never_downloaded_again(cache, monkeypatch):
    ...
    _seed(cache, (40.0, -110.0), {p: _flat(20.0) for p in PARAMETERS})
    assert NasaPowerClimateSource(cache).fetch((FORT_COLLINS,))["humidity"][
        "08069"
    ] == pytest.approx(20.0)
...
def test_a_two_hundred_carrying_an_error_body_is_not_read_as_data(cache, monkeypatch):
    ...
    with pytest.raises(ValueError, match="not available"):
        source._request_once("T2M_MAX", 40.0, -110.0)
...
def test_a_failing_tile_is_recorded_and_the_others_still_arrive(cache, monkeypatch):
    ...
    source = NasaPowerClimateSource(cache, retries=1)
    values = source.fetch((FORT_COLLINS, MIAMI))
    assert values["humidity"]["08069"] == pytest.approx(20.0)
...
def test_an_empty_tile_is_not_written_to_the_cache(cache, monkeypatch):
    ...
    source = NasaPowerClimateSource(cache, retries=1)
    assert source._tile("T2M_MAX", (40.0, -110.0)) == {}
    assert not cache.has(f"power-T2M_MAX-40--110-t{TILE:g}", ".json")
